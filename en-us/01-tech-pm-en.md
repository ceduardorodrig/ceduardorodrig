---
tags: [meta, docs, personal]
---

# 👤 Carlos Eduardo Rodrigues

**Data & Product Architect | AI Governance | Founder @ Sumænimá**

[ceduardorodrig@gmail.com](mailto:ceduardorodrig@gmail.com) | +55 (61) 9-9803-3546 | Brasília-DF, Brazil

[linkedin.com/in/c-eduardo-rodrigues](https://linkedin.com/in/c-eduardo-rodrigues) | [github.com/ceduardorodrig](https://github.com/ceduardorodrig) | [sumaenima.chimaera-heptatonic.ts.net](https://sumaenima.chimaera-heptatonic.ts.net)

---

## 🛠️ Skills

| Area | Skills |
|------|--------|
| **Product Management** | Roadmap & Strategy, Product Discovery, Backlog Prioritization, Minimum Viable Product (MVP) Definition, OKRs, User Stories |
| **Data & Analytics** | Product Metrics, Google Analytics, A/B Testing, Data-Driven Decision Making, Dashboards |
| **Frameworks & Methods** | Agile/Scrum (Scrum Master), Jobs-to-be-Done, Stakeholder Mapping (RACI), Gantt, Sprint Planning |
| **Research & UX** | Ethnography, Qualitative Research, User Interviews, UX Research, Participant Observation |
| **Technology** | Python, SQL, React, PostgreSQL, Docker, FastAPI, Git, Local AI (Whisper, LLMs) |
| **Domain** | Socio-Environmental Impact, Public Policy, Traditional Communities, Territorial Data |

---

## 👤 Profile

Product Manager with a track record in data products and socio-environmental impact. Built and shipped a SaaS platform from zero to production — defining roadmap, prioritizing backlog, conducting user discovery, and validating MVPs (Minimum Viable Products) with institutional clients (IPEA, CNPCT/Planalto, IEB). Led data-driven digital strategy achieving +143% growth and 2M+ reach, managed US$ 10K/month budget as Scrum Master, and conducted product discovery that shaped a national platform adopted by Brazil's Federal Public Ministry. Anthropology background provides deep user empathy and ability to translate qualitative insights into product requirements. Seeking first formal Product Manager (PM) role in a team with senior mentorship to accelerate growth.

---

## 💼 Experience

### 🚀 Sumænimá — Founder & Product Owner
*2016 — present · Brasília-DF / Remote*

- **Led** end-to-end product lifecycle for StênioBOT, defining roadmap and prioritizing features across 4 modules based on field discovery with 5+ institutional partners
- **Delivered** real-time AI transcription product validated at highest government level (CNPCT/Planalto), serving 30+ stakeholders per session
- **Orchestrated** multi-stakeholder reporting engagements (IPEA — 3-day strategic planning, IEB — national youth meeting, CNPCT — government council), translating qualitative research needs into product requirements
- **Managed** product backlog and delivery roadmap across a 30-container SaaS platform (FastAPI + React + PostgreSQL + Docker) — from concept through field validation to production
- **Drove** product discovery with traditional communities and institutional clients, applying Jobs-to-be-Done approach to define MVP scope and prioritize features under resource constraints

### 🌳 IPAM — Amazon Environmental Research Institute
*2022 — 2025 · Brasília-DF, Brazil*

**Digital Product Lead** (2024–2025) · **Communications Assistant** (2023–2024) · **Trainee** (2022–2023)

- **Drove** data-driven digital strategy achieving +143.3% audience growth on Facebook and +100% on Instagram across 5 channels, reaching 2M+ users with 66.7K interactions
- **Led** cross-functional team of designers, writers and analysts as Scrum Master, implementing sprint planning, retrospectives and delivery metrics
- **Managed** US$ 10K/month ad grants budget with continuous A/B testing, audience segmentation and data-informed optimization
- **Delivered** award-winning campaign recognized with Mercosur Scientific Journalism Award — 1st place, Social Networks (2024)
- **Defined** product KPIs and OKRs for digital channels, treating social media as data products with integrated organic + paid funnels

### 🗺️ ISPN — Institute for Society, Population and Nature
*2017 — 2021 · Brasília-DF, Brazil*

**Junior Technical Advisor** (2019–2021) · **Trainee** (2017–2019)

- **Conducted** product discovery with 50+ traditional Cerrado communities, applying ethnographic methods to uncover unmet needs in territorial mapping
- **Translated** field research into product requirements adopted by Brazil's Federal Public Ministry (MPF), now empowering thousands of users
- **Defined** MVP scope and user flows for a self-mapping platform addressing a critical data gap (official data covered only 28% of communities)
- **Collaborated** with cross-functional team (anthropologists, developers, designers) from problem definition through prototype validation

### 🦎 Participatory Fauna Monitoring Network
*2020 — 2022 · Cavalcante-GO, Brazil*

**Stakeholder Facilitator**

- **Facilitated** multi-institutional working group (ICMBio, UnB, World Bank, landowners, traditional communities) to define shared roadmap for a wildlife corridor
- **Mapped** stakeholder responsibilities (RACI-like) across 5+ organizations, aligning competing priorities toward common conservation outcome
- **Delivered** executive report synthesizing diverse stakeholder perspectives into actionable project plan

### ⚖️ APIB — Articulation of Indigenous Peoples of Brazil
*2020 · Remote*

- **Centralized** and systematized COVID-19 data across Indigenous Peoples in Brazil during public health crisis, applying data governance under urgent constraints

---

## 🎓 Education

**University of Brasília (UnB)** — Bachelor's Degree in Social Sciences / Anthropology (2016–2023)

**Master's in Anthropology** — UnB (2024–2025, 2 semesters completed)
Left the program to dedicate myself fully to product management and systems architecture.

---

## 📚 Publications

- **Co-author** — Moser et al. *"Institutional Invisibility Threatens the Lands and Livelihoods of Traditional Communities in the Northern Brazilian Cerrado"* — Land Use Policy (Elsevier, 2026)
- **Article** — Rodrigues, C.E. *"Doenças são sintomas de uma crise cultural e ambiental"* — JOTA (2026)
- **Documentary** — *Manaus Extrema* (2024) — Co-direction. IPAM. Mercosur Scientific Journalism Award

---

## 📡 Infrastructure

Own infrastructure for data processing and storage: multi-node cluster orchestrated via Docker Swarm with PostgreSQL, cache, monitoring, and web edge services. Local processing with guaranteed data sovereignty. 30+ containers in production.

---

## 🌐 Languages

- **Portuguese** — Native
- **English** — Fluent (reading, writing, technical and academic conversation)
